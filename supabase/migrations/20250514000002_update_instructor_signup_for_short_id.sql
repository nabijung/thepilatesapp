...
BEGIN
  ...
  BEGIN
    ...
    IF p_studio_name IS NOT NULL THEN
      -- Generate a short ID for the studio
      v_short_id := generate_short_id();

      -- Create new studio with auto-generated UUID and short_id
      INSERT INTO studio (studio_id, name, location, short_id)
      VALUES (gen_random_uuid(), p_studio_name, COALESCE(p_studio_location, ''), v_short_id)
